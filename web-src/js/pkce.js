// Minimal SHA-256 for PKCE when crypto.subtle is unavailable (file:// pages)
function sha256Fallback(message) {
  ...
  withLen.set(bytes);
  withLen[l] = 0x80;
  new DataView(withLen.buffer).setUint32(withLen.length - 4, l * 8, false);
  const H = new Uint32Array([0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19]);
  const W = new Uint32Array(64);
  for (let i = 0; i < withLen.length; i += 64) {
    for (let t = 0; t < 16; t++) W[t] = new DataView(withLen.buffer, i + t * 4, 4).getUint32(0, false);
    for (let t = 16; t < 64; t++) {
