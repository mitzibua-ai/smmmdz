...
from pccheck.utils.match import is_whitelisted_path, match_path, suspicious_filename
...
def _match_suspicious_path(path: str) -> tuple[str, str, Severity, Category] | None:
    ...
    for sig in all_sigs:
        for pattern in sig.patterns:
            if len(pattern) >= 5 and match_path(pattern, path):
                return sig.name, pattern, sig.severity, sig.category
            if len(pattern) == 4 and pattern.lower() in {"9z"}:
                if re.search(r"(^|[^a-z0-9])9z([^a-z0-9]|$)", lower):
                    return sig.name, pattern, sig.severity, sig.category
    name = Path(path).name if path else ""
    for sus in ("9zcleaner", "cheat", "injector", "bypass", "spoofer", "eulen", "susano", "macho"):
        if suspicious_filename(name, sus):
