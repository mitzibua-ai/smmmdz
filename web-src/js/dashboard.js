          ...
          <div class="tool-brand__grid">
            <div class="tool-brand__stage">
              <div class="tool-brand__window" aria-hidden="true">
                ...
                <div class="tool-brand__window-body tool-brand__window-body--cover${hasCustom ? " has-cover" : ""}" id="tool-brand-hero" ${coverStyle}>
                  <div class="tool-brand__cover-wash"></div>
                  <div class="tool-brand__cover-content">
                  ...
                  ${hasCustom ? "" : `<div class="tool-brand__preview-empty">Upload a PNG, JPG, or GIF<br/><span>covers the whole window</span></div>`}
                </div>
              </div>
              <p class="tool-brand__hint">Live preview — background fills the full tool window</p>
            </div>

            <div class="tool-brand__controls">
              <div class="tool-brand__block">
                <div class="tool-brand__block-title">Custom image or GIF</div>
                <p class="tool-brand__block-sub">Covers the entire PC Check window as the background.</p>
                ...
                <p class="tool-brand__file-name" id="tool-brand-file-name">${escapeHtml(branding.customImageName || (hasCustom ? "Custom background" : "No image uploaded"))}</p>
...
const FREE_TOOLS_CATALOG = [
  { id: "autoruns", name: "Autoruns", tag: "Startup", externalUrl: "https://download.sysinternals.com/files/Autoruns.zip", description: "Sysinternals startup and persistence viewer — services, Run keys, scheduled tasks, and more." },
  { id: "string-explorer", name: "Strings", tag: "PE Analysis", externalUrl: "https://download.sysinternals.com/files/Strings.zip", description: "Sysinternals Strings — extract ASCII/Unicode strings from executables and dumps." },
  { id: "prefetch", name: "Prefetch Parser", tag: "Prefetch", externalUrl: "https://github.com/spokwn/prefetch-parser/releases/download/v1.5.3/PrefetchParser.exe", description: "Prefetch execution timeline parser for Windows Prefetch artifacts." },
  { id: "usbdeview", name: "USBDeview", tag: "USB / DMA", externalUrl: "https://www.nirsoft.net/utils/usbdeview-x64.zip", description: "NirSoft USB device history — plugged devices, serials, driver info, and last plug times." },
  { id: "saved-files", name: "Recent Files View", tag: "Downloads", externalUrl: "https://www.nirsoft.net/utils/recentfilesview.zip", description: "NirSoft RecentFilesView — files recently opened or saved on this PC." },
  { id: "powershell", name: "Hayabusa", tag: "PowerShell", externalUrl: "https://github.com/Yamato-Security/hayabusa/releases/download/v3.1.1/hayabusa-3.1.1-win-x64.zip", description: "Yamato-Security Hayabusa — Windows event log forensics with PowerShell and sigma-style rules." },
  { id: "paths", name: "Search My Files", tag: "Paths", externalUrl: "https://www.nirsoft.net/utils/searchmyfiles-x64.zip", description: "NirSoft SearchMyFiles — fast multi-criteria file search across drives and folders." },
  { id: "mft", name: "AlternateStreamView", tag: "MFT / ADS", externalUrl: "https://www.nirsoft.net/utils/alternatestreamview.zip", description: "NirSoft AlternateStreamView — scan NTFS Alternate Data Streams on disks and folders." },
  { id: "kernel-dump", name: "Process Parser", tag: "Process", externalUrl: "https://github.com/spokwn/process-parser/releases/download/v0.5.3/ProcessParser.exe", description: "Process execution artifact parser for investigating launched binaries and related traces." },
  { id: "journal", name: "JournalTrace", tag: "USN Journal", externalUrl: "https://github.com/spokwn/JournalTrace/releases/download/1.2/JournalTrace.exe", description: "USN Journal viewer for file create/delete/rename activity on NTFS volumes." },
  { id: "crash", name: "AppCrashView", tag: "Crash Logs", externalUrl: "https://www.nirsoft.net/utils/appcrashview.zip", description: "NirSoft AppCrashView — Windows Error Reporting crash dumps and application fault history." },
  { id: "browser-history", name: "BrowsingHistoryView", tag: "Browser", externalUrl: "https://www.nirsoft.net/utils/browsinghistoryview-x64.zip", description: "NirSoft multi-browser history viewer for Chrome, Edge, Firefox, and more." },
  { id: "browser-downloads", name: "BrowserDownloadsView", tag: "Browser", externalUrl: "https://www.nirsoft.net/utils/browserdownloadsview-x64.zip", description: "NirSoft aggregated browser download history across major browsers." },
  { id: "bam", name: "BAM Parser", tag: "Execution", externalUrl: "https://github.com/spokwn/BAM-parser/releases/download/v1.2.6/BAMParser.exe", description: "Background Activity Moderator parser — last execution paths and timestamps." },
  { id: "amcache", name: "Executed Programs", tag: "Amcache", externalUrl: "https://www.nirsoft.net/utils/executedprogramslist.zip", description: "NirSoft ExecutedProgramsList — programs previously run on the system with install paths." },
  { id: "srum", name: "Network Usage View", tag: "Network", externalUrl: "https://www.nirsoft.net/utils/networkusageview.zip", description: "NirSoft NetworkUsageView — Windows network usage data similar to SRUM-style activity." },
  { id: "recent-activity", name: "Recent Activity", tag: "Activity", externalUrl: "https://www.nirsoft.net/utils/lastactivityview.zip", description: "NirSoft LastActivityView — recent file, app, and system activity timeline." },
  { id: "recently-deleted", name: "Recently Deleted", tag: "Deleted", externalUrl: "https://www.nirsoft.net/utils/previousfilesrecovery.zip", description: "NirSoft PreviousFilesRecovery — recover recently deleted files from NTFS volumes." },
  {
    id: "everything",
    name: "Everything",
    tag: "Search",
    ...
    description: "voidtools Everything — instant filename search across the whole drive.",
    badge: "NEW",
  },
  {
    id: "osforensics",
...
function _toolBrandSetStatus(text, isError = false) {
  const el = $("tool-brand-status");
  if (!el) return;
  el.textContent = text || "";
...
function _refreshToolBrandPreview() {
  ...
  if (hero) {
    hero.classList.toggle("has-cover", hasCustom);
    if (hasCustom) {
      hero.style.backgroundImage = `url('${branding.customImage}')`;
      hero.querySelector(".tool-brand__preview-empty")?.remove();
    } else {
      hero.style.backgroundImage = "";
      if (!hero.querySelector(".tool-brand__preview-empty")) {
        const empty = document.createElement("div");
        empty.className = "tool-brand__preview-empty";
        empty.innerHTML = "Upload a PNG, JPG, or GIF<br/><span>covers the whole window</span>";
  ...
  if (nameEl) {
    nameEl.textContent = branding.customImageName || (hasCustom ? "Custom background" : "No image uploaded");
  }
  if (label) {
...
function bindToolBrandingEvents() {
  ...
  $("tool-brand-download")?.addEventListener("click", async () => {
    ...
    try {
      ...
      _toolBrandSetStatus(err.message || "Download failed.", true);
      alert(err.message || "Could not build branded EXE.");
    } finally {
      btn.disabled = false;
      btn.textContent = "Download branded EXE";
    }
  });
}
